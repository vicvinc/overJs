...
/***/ function(module, exports, __webpack_require__) {
	...
	* @Last Modified time: 2015-09-24 16:32:23
	*/
	...
	var _libUtilsJs = __webpack_require__(1);

	var _libUtilsJs2 = _interopRequireDefault(_libUtilsJs);
	...
	var CommentBox = React.createClass({
		...
		loadCommentsFromServer: function loadCommentsFromServer() {
			_libUtilsJs2['default'].httpGet(this.props.url, '', (function (xhr, status, err) {
		...
		handleCommentSubmit: function handleCommentSubmit(comment) {
			...
			_libUtilsJs2['default'].httpPost(this.props.url, comment, (function (xhr, status, err) {
				console.error(this.props.url, status, err.toString());
			}).bind(this), (function (data) {
				this.setState({ data: data });
			}).bind(this));
	...
	React.render(React.createElement(CommentBox, { url: 'comments.json', pollInterval: 2000 }), document.getElementById('app'));
...
/***/ function(module, exports) {
	...
	/* 
	* @Author: Administrator
	* @Date:   2015-09-24 15:24:13
	* @Last Modified by:   Administrator
	* @Last Modified time: 2015-09-24 15:24:19
	*/
	...
	var utils = {
	    httpGet: function httpGet(url, params, callback, complete) {
	    ...
	    httpPost: function httpPost(url, params, callback, complete) {
	        var xmlhttp = new XMLHttpRequest();
	    ...
	    concatParams: function concatParams(url, params) {
	        if (url.match(/\?/)) {
	            var str = '&';
	        } else {
	            var str = '?';
	    ...
	    setCookie: function setCookie(key, value, expires, path) {
	        var exp = new Date();
	        var path = path || '/';
	    ...
	    transBytes: function transBytes(bytes) {
	    ...
	    transTimes: function transTimes(timeStamp) {
	    ...
	    insertAfterText: function insertAfterText(newElement, targetElement) {
	        var parent = targetElement.parentNode;
	        if (parent.lastChild == targetElement) {
	            parent.appendChild(newElement);
	        } else {
	    ...
	    extend: function extend(src, obj) {
	        for (var key in obj) {
	            if (!src[key]) {
	                src[key] = obj[key];
	            }
